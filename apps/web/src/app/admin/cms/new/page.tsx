...
export default function NewCmsBlockPage() {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      await apiFetch(`/api/v1/admin/cms/blocks`, { method: "POST", body: JSON.stringify(body) });
  ...
  return (
    <div className="p-6 space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">New CMS Block</h1>
          <p className="text-sm text-slate-500 mt-1">Create a content block for use in report templates</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={onSave} disabled={saving} className="px-4 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 font-medium disabled:opacity-50">{saving ? 'Saving…' : 'Save Block'}</button>
        </div>
      </div>
